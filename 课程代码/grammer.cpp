...
//    vector<Operand> operand;//操作数数组
//    list<Quaternary>::iterator quater;
...
//	lastch = ch;//记录上一位单词
//	SCANNER();//词法扫描器读取一个单词
//    ch = --TOKEN_list.end();//获取该单词的TOKEN流迭代器
//    cout << (*ch).token << endl;
//    if(ch == lastch)end = 1;//若无新的单词，则代表读取完成，结束符号end置1
	if(begin)
	{
		ch = TOKEN_list.begin();
		begin = 0;
...
{
//    string s;
//    for(int i=2;i<(*ch_ope).token.size();i++)
//        s+=(*ch_ope).token[i];
//    stringstream st;
//    st<<s;
//    st>>mark;
...
{
//    if((*ch_ope).token == "OT16"||(*ch_ope).token == "OT20")
//    {//!,~
//        GetMark(ch_ope);
//    }
	if((*ch_ope).token == "OT5")
...
int GrammaticalAnalysis::Function()//函数
{
    ...
    {//Identifier 判断是否是i4，是则读入下一个词，不是则返回值为0
        ch_function=ch;
        ch_sem=ch_function;
...
//                    id_now->tp->type->rinfl=id_struct->tp->type->rinfl;
//                    id_now->tp->lenl->length=id_struct->tp->lenl->length;
                    if(!end &&(*ch).token == "PT8")
                    {//;
                        nextW();
                        cout<<(*(id_now)).name;
    ...
    if(!end&&(*ch).token=="IT")
    {
        ...
        else
        {
            ...
            if(!Id_Expression())
            {
                ...
                if(!end&&(*ch).token=="OT7")
                {
                   // SEM_stack.push_back(ch_sem);//若为赋值，则将=之前的标识符入栈，否则不入
                    ch_ope=ch;
                    nextW();
                    if(!Logical_or()){return 0;}
                    else
                    {
                        GetMark(ch_ope);/*学长用来获取运算符*/
                        Assign_oper_qua();/*赋值操作数生成*/
                        return 1;
                    }
                }
